...
tournament.cut_text       # => "The following players did not make match play"
tournament.rounds         # => [{ id: 2001, name: "R2", in_progress: false }, ...]

# Access round metadata
tournament.rounds.each do |round|
  puts "#{round[:name]} (#{round[:id]})"
end
```
      ...
      <tr>
        <% tournament.columns.each do |col| %>
          <%
            ...
            css_classes << "round-#{col.round_id}" if col.round_id
            css_classes << "summary" if col.round_id.nil?
          %>
...
<table class="alpha-board">
  ...
  <tbody>
    <% tournament.rows.sort_by(&:name).each do |row| %>
      <tr>
        <% row.cells.each do |cell| %>
          <td><%= cell %></td>
        <% end %>
...
- `cut_text` → String|nil
- `adjusted?` → Boolean
- `rounds` → Array<Hash> - Round metadata (alias: `rounds_meta`)
...
- `affiliation` → String|Array<String>|nil
- `cut?` → Boolean
...
```ruby
# Get only players who made the cut
tournament.rows.select { |r| !r.cut? }

# Filter by position
tournament.rows.select { |r| r.summary[:position] == "1" }

# Sort by name
tournament.rows.sort_by(&:name)

# Sort by score (custom logic for golf scores)
tournament.rows.sort_by do |row|
  score = row.summary[:total_to_par_gross]
  case score
  when "E" then 0
  when /^[+-]\d+$/ then score.to_i
  when /^T(\d+)$/ then $1.to_i
  else Float::INFINITY
  end
end
```
...
columns = tournament.columns.select { |c| ["position", "player", "total-to-par-gross"].include?(c.format) }

# Remove "thru" columns
columns = tournament.columns.reject { |c| c.format == "thru" }

# Get summary columns only
columns = tournament.columns.select { |c| c.round_id.nil? }

# Get columns for a specific round
columns = tournament.columns.select { |c| c.round_id == 2001 }
```
